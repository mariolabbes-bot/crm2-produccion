...
async function processStockFileAsync(jobId, filePath, originalname) {
    ...
    try {
        ...
        let colSku = headers.find(h => /C.*DIGO/i.test(String(h).trim()));
        if (!colSku) colSku = headers.find(h => /SKU/i.test(String(h).trim()));
        if (!colSku) colSku = headers.find(h => /ART.*CULO/i.test(String(h).trim()));
